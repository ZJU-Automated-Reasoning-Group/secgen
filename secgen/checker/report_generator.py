...
class ReportGenerator:
    ...
    def convert_severity(self, severity_str: str) -> Severity:
        """Convert string severity to Severity enum."""
        return getattr(Severity, severity_str.upper())
    
    def convert_vuln_types(self, type_strings: List[str]) -> set:
    ...
    def format_output(self, report: AnalysisReport, detector: Any, format_type: str) -> str:
        """Format analysis report based on format type."""
        if format_type == "json":
            return json.dumps(self.export_json_report(report), indent=2)
        elif format_type == "sarif":
            return self.format_sarif_output(report, detector)
        else:  # text
            return self.generate_summary_report(report)
    
    def format_sarif_output(self, report: AnalysisReport, detector: Any) -> str:
    ...
    def list_vulnerability_types(self):
