...
from secgen.agent.minotor import TokenUsage
from secgen.agent.tools import Tool
from secgen.utils import RateLimiter, _is_package_available, encode_image_base64, make_image_url, parse_json_blob
        ...
        if stream_delta.tool_calls:
            for tool_call_delta in stream_delta.tool_calls:  # ?ormally there should be only one call at a time
                ...
                if tool_call_delta.index is not None:
                    ...
                    logger.warning(f"Tool call index is not provided in tool delta: {tool_call_delta}")
                    continue
...
def get_clean_message_list(
    ...
    for message in message_list:
        ...
        if role not in MessageRole.roles():
            logger.error(f"Incorrect role {role}, only {MessageRole.roles()} are supported for now.")
...
class TransformersModel(Model):
    ...
    def make_stopping_criteria(self, stop_sequences: list[str], tokenizer) -> Any:
